...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        ...
        {
            arr_small[count2++] = arr_big[i];	/*把字母出现的次数输入到count2*//*补充注释（说明作用）*/
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  /*没有左右子树*//*补充判断模块注释*/
        {
            length = findParent(i,huffman,count);   //得到各个叶结点的带权路径长度
            sum+=length*huffman[i][3];/*自行填充代码*/  	//各个叶结点带权路径长度累加得到总长度WPL
        }
    }
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  /*跳过没出现的字母*//*补充判断模块注释（说明作用）*/
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  /*求较小值*//*补充判断模块注释（说明作用）*/
        {
            min = huffman[i][3];
            ii[0] = i;
        }
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        ...
        if(huffman[i][3] < min2 && huffman[i][0] == -1 && i!=ii[0])  /*求较小值*//*补充判断模块注释（说明作用）*/
        {
            min2 = huffman[i][3];
            ii[1] = i;
        }
...
{
    int length = 0;
    if(huffman[i][0] == -1)    /*没有父节点*//*补充判断模块注释*/
    {
        return 0;
    }
